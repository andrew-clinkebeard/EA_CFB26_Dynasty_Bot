...
def create_recruit_spotlight(title, subtitle, author, recap_text, output_folder=".", background_color=HexColor("#EFEECE"), image_path="./Graphics/logo_dynasty_tribune.png"):
    ...
    top_frame_height = pageSize[1] - (doc.topMargin + doc.bottomMargin)
    frame_height = pageSize[1] - doc.topMargin - doc.bottomMargin

    #top left
    frame1 = Frame(doc.leftMargin, doc.bottomMargin, frame_width, frame_height, id='topleft', showBoundary=1)
    #top right
    frame2 = Frame(doc.leftMargin + frame_width + 0.25 * inch, doc.bottomMargin, frame_width, frame_height, id='topright', showBoundary=1)
    #bottom
    frame3 = Frame(doc.leftMargin, doc.bottomMargin, bottome_frame_width, frame_height, id='bottom', showBoundary=1)
...
"""
    ...
    create_recruit_spotlight(
        ...
        recap_text=recapText,
        output_folder=output_folder, 
        background_color=HexColor("#EFEECE")
    )
